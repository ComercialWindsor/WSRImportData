...
public class InvoiceGenXML extends GenForm
{
	...
	public String generate(IStatusBar statusBar)
	{
		...
		{
			info = generateXML(trxName, trx ,selectedId);
	...
	private String generateXML (String trxName, Trx trx,int C_Invoice_ID){		
		try {
			MXMLInvoice inv = new MXMLInvoice(Env.getCtx(), C_Invoice_ID, trxName);
			inv.generateXML();
		} catch (Exception e) {
