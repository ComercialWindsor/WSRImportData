 ...
 */
public class ExportDTEInvoiceFOLOrgMassive extends SvrProcess
{	
	/** Properties						*/
...
String sql = "select C_Invoice_ID from covid_invoice where ispaid='N'";	
		...
		try
		{
			...
			while (rs.next())
			{
				...
				MInvoice inv=new MInvoice(m_ctx,rs.getInt("C_Invoice_ID"),get_TrxName());
		...
		}//catch select facacct
		finally
		{
						
						
						
		} // finally select factacct
